...
Note that the file has lots of funny characters, so we're going to have to read it the old-fashioned way, char-by-char.

```{r,  message = FALSE, warning = FALSE}
library(tidyverse)

## given a file path 'raw_file', produces the useful dataframes glucose_raw and activity_raw

raw_file <- file.path(Sys.getenv("ONEDRIVE"),"Sprague 2019/Health 2019/Librelink.csv")
colnames_librelink <- c("meter", "serial_number", "meter_timestamp", "record_type", "historic_glucose", "scan_glucose",
                        ...
                        "meal_insulin", "correction_insulin", "user_change_insulin")

libre_raw <- read.csv(raw_file,
                             skip = 2, col.names = colnames_librelink)
libre_raw$meter_timestamp <- lubridate::force_tz(lubridate::mdy_hm(libre_raw$meter_timestamp), "US/Pacific")

glucose <- libre_raw %>% select(meter_timestamp,scan_glucose, historic_glucose, strip_glucose)

#glucose$time <- readr::parse_datetime(libre_raw$`Meter Timestamp`,locale = locale(tz="US/Pacific"))

glucose$time <- lubridate::as_datetime(libre_raw$meter_timestamp, tz = "US/Pacific")
# 
glucose$value <- dplyr::if_else(is.na(glucose$scan),glucose$hist,glucose$scan)
# apply correction for faulty 2019-03-24 sensor
glucose$value <- dplyr::if_else(glucose$time>lubridate::as_datetime("2019-03-15"),glucose$value+20,as.numeric(glucose$value))


glucose_raw <- glucose

activity_raw <- data_frame(Start=lubridate::now(), End = lubridate::now(), Activity = "", Comment = "", Z = 0)
